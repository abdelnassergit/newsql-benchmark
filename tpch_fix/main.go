...
func newDB(targets []string, driver string, user string, password string, dbName string, connParams string) (*sql.DB, error) {
	...
	for i, addr := range targets {
		...
		switch driver {
		case mysqlDriver:
			 // Check if we're connecting to TiDB
            if strings.Contains(connParams, "tidb_") {
                // TiDB specific DSN
...
func openDB() {
    ...
    // Try to connect without database first with basic parameters
    basicConnParams := ""
    if driver == mysqlDriver && !strings.Contains(connParams, "tidb_") {
        // Use basic MySQL parameters only
        basicConnParams = "sql_mode='STRICT_TRANS_TABLES'"
    } else {
        basicConnParams = connParams
    }

    tmpDB, err = newDB(targets, driver, user, password, "", basicConnParams)
    if err != nil {
        panic(fmt.Errorf("failed to connect to server: %v", err))
    }
    defer tmpDB.Close()

    // Check if database exists - adjust query based on driver
    var exists int
    var query string
    switch driver {
    case mysqlDriver:
    ...
    if err := tmpDB.QueryRow(query, dbName).Scan(&exists); err != nil {
        if strings.Contains(err.Error(), "Unknown system variable 'tidb_") {
            // This is MySQL, not TiDB - try simpler approach
            fmt.Println("Warning: Running on MySQL instead of TiDB - using simplified database creation")
            _, err = tmpDB.Exec(fmt.Sprintf("CREATE DATABASE IF NOT EXISTS %s", dbName))
            if err != nil {
                panic(fmt.Errorf("failed to create database: %v", err))
            }
        } else {
            panic(fmt.Errorf("failed to check database existence: %v", err))
        }
    }
    ...
    // Connect to the specific database with appropriate parameters
    if driver == mysqlDriver && !strings.Contains(connParams, "tidb_") {
        // Use basic MySQL parameters
        connParams = "sql_mode='STRICT_TRANS_TABLES'"
    }
