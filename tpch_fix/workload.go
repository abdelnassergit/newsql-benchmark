...
func (w *Workloader) Prepare(ctx context.Context, threadID int) error {
	...
	// Check if we're running on MySQL and adjust configuration
	if w.cfg.Driver == "mysql" {
		// Test if this is TiDB by checking for TiDB specific variables
		var version string
		err := w.db.QueryRow("SELECT @@version").Scan(&version)
		if (err != nil) {
			return fmt.Errorf("failed to get database version: %v", err)
		}
		
		// If not TiDB, adjust configuration
		if !strings.Contains(strings.ToLower(version), "tidb") {
			fmt.Println("Warning: Running on MySQL instead of TiDB - some features may not be available")
			// Skip TiDB specific configurations
			w.cfg.TiFlashReplica = 0
			w.cfg.EnableQueryTuning = false
		}
	}
...
func (w *Workloader) verifyDBState(ctx context.Context) error {
	s := w.getState(ctx)
	
	// Verify database existence
	var dbExists bool
	err := s.Conn.QueryRowContext(ctx, "SELECT 1 FROM information_schema.schemata WHERE schema_name = ?", w.cfg.DBName).Scan(&dbExists)
	if err != nil {
		return fmt.Errorf("failed to check database existence: %v", err)
	}
	if !dbExists {
		return fmt.Errorf("database %s does not exist. Please run 'prepare' command first", w.cfg.DBName)
	}

	// Verify required tables
	requiredTables := []string{"nation", "region", "part", "supplier", "partsupp", "customer", "orders", "lineitem"}
	for _, table := range requiredTables {
		var exists bool
		err := s.Conn.QueryRowContext(ctx, 
			"SELECT 1 FROM information_schema.tables WHERE table_schema = ? AND table_name = ?", 
			w.cfg.DBName, table).Scan(&exists)
		if err != nil {
			return fmt.Errorf("failed to check table %s existence: %v", table, err)
		}
		if !exists {
			return fmt.Errorf("table %s does not exist in database %s. Please run 'prepare' command first", 
				table, w.cfg.DBName)
		}
	}
...
func (w *Workloader) Run(ctx context.Context, threadID int) error {
	...
	// Verify database state on first thread only
	if threadID == 0 {
		if err := w.verifyDBState(ctx); err != nil {
			return err
		}
	}
	
	// Add ping with retry logic
	maxRetries := 3
